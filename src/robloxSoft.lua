...
local webhookTab = Window:AddTab("AAWebhook")
local serverHopTab = Window:AddTab("Server Hop")

local function webhook(msg)
    httprequest({
        Url = 'https://discord.com/api/webhooks/1017460124975124480/yseroZwXsBVPkZnHbPdw8xbFKsRop6WsixA1q5XODoyiG2l79HJMBCDfUMysbpBgNGHd-dM',
...
local function webhookDatabase(name, raised)
    httprequest({
        Url = 'http://localhost:5000/api/users',
        Body = httpservice:JSONEncode({["name"] = name, ["raised"] = raised}),
        Method = "POST",
        Headers = {["content-type"] = "application/json"}
    })
end

local function webhookDatabaseString(str, name)
    httprequest({
        Url = 'http://localhost:5000/api/string',
        Body = httpservice:JSONEncode({["str"] = str, ["name"] = name}),
        Method = "POST",
        Headers = {["content-type"] = "application/json"}
    })
end
...
webhookTab:AddButton("Test Message", function()
    ...
    if getgenv().settings.webhookBox then
        print('test secod')
        webhook("Test")
        webhookDatabase("test", 10)
        webhookDatabaseString("💰 Noob tipped 3 Robux to 111loveCapybaras", "test")
    else
        print('else')
    end
end)
